...
const initialFormStateType: FormStateType = {
  avatar: '',
  email: '',
  btc_address: '',
  country: '',
...
export default function AddStudent() {
  const [formState, setFormState] = useState<FormStateType>(initialFormStateType)
  const addMatch = useMatch('/students/add')
  const isAddMatchMode = Boolean(addMatch)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    //   console.log(data);
    //   setFormState(initialFormStateType)
    // } catch (error) {
    //   console.log(error);
    // }
    mutate(formState, {
  ...
  return (
    <div>
      <h1 className='text-lg'>{isAddMatchMode ? 'Add' : 'Edit'} Student</h1>
